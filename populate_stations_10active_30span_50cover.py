...
## and saves as stations_10active_30span_50cover.csv
import os
import psycopg2
...
def fetch_stations():
        ...
        FROM weather.stations_raw sr\
            WHERE (sr.station_jsonb ->> \'maxdate\')::date >= CURRENT_DATE - INTERVAL \'10 years\'\
                AND (sr.station_jsonb ->> \'maxdate\')::date - INTERVAL \'30 years\' >= (sr.station_jsonb ->> \'mindate\')::date\
                    AND (sr.station_jsonb ->> \'datacoverage\')::DECIMAL > 0.5'
    cur.execute(query)
    ...
    # df_cc = add_cc(df)
    df.to_csv('/home/theraceblogger/weather-db-example/stations_10active_30span_50cover.csv', index=False)
